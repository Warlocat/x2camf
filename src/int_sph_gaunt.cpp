...
int2eJK INT_SPH::get_h2e_JK_gaunt(const string& intType, const int& occMaxL) const
{
    ...
    {
        int l_q = shell_list[qshell].l, l_max = max(l_p,l_q), LmaxJ = 1, LmaxK = l_p+l_q+1;
        ...
        for(int twojj_q = abs(2*l_q-1); twojj_q <= 2*l_q+1; twojj_q = twojj_q + 2)
        {
            ...
            for(int tmp = LmaxJ; tmp >= 0; tmp -= 2)
            {
                ...
                for(int mq = 0; mq < twojj_q + 1; mq++)
                {
                    if(intType.substr(0,4) == "LSLS")
                        array_angular_J[tmp][index_tmp_p][index_tmp_q][mp*(twojj_q + 1)+mq] = int2e_get_angular_gaunt_LSLS(l_p, 2*mp-twojj_p, sym_ap, l_p, 2*mp-twojj_p, sym_ap, l_q, 2*mq-twojj_q, sym_aq, l_q, 2*mq-twojj_q, sym_aq, tmp);
                    else if(intType.substr(0,4) == "LSSL")
                        array_angular_J[tmp][index_tmp_p][index_tmp_q][mp*(twojj_q + 1)+mq] = int2e_get_angular_gaunt_LSSL(l_p, 2*mp-twojj_p, sym_ap, l_p, 2*mp-twojj_p, sym_ap, l_q, 2*mq-twojj_q, sym_aq, l_q, 2*mq-twojj_q, sym_aq, tmp);
                    else
                    {
            ...
            for(int tmp = LmaxK; tmp >= 0; tmp -= 2)
            {
                ...
                {
                    if(intType.substr(0,4) == "LSLS")
                        array_angular_K[tmp][index_tmp_p][index_tmp_q][mp*(twojj_q + 1)+mq] = int2e_get_angular_gaunt_LSLS(l_p, 2*mp-twojj_p, sym_ap, l_q, 2*mq-twojj_q, sym_aq, l_q, 2*mq-twojj_q, sym_aq, l_p, 2*mp-twojj_p, sym_ap, tmp);
                    else if(intType.substr(0,4) == "LSSL")
                        array_angular_K[tmp][index_tmp_p][index_tmp_q][mp*(twojj_q + 1)+mq] = int2e_get_angular_gaunt_LSSL(l_p, 2*mp-twojj_p, sym_ap, l_q, 2*mq-twojj_q, sym_aq, l_q, 2*mq-twojj_q, sym_aq, l_p, 2*mp-twojj_p, sym_ap, tmp);
                    else
                    {
                        cout << "ERROR: Unkonwn intType in get_h2e_JK_gaunt." << endl;
                        exit(99);
        ...
        for(int tt = 0; tt < size_gtos_p*size_gtos_p*size_gtos_q*size_gtos_q; tt++)
        {
            ...
            for(int twojj_q = abs(2*l_q-1); twojj_q <= 2*l_q+1; twojj_q = twojj_q + 2)
            {
                ...
                for(int tmp = LmaxJ; tmp >= 0; tmp -= 2)
                {
                    if(intType == "LSLS")
                    {
                        array_radial_J[tmp][e1J][e2J][index_tmp_p][index_tmp_q] = 4.0*a2*a4 * radial_2e_list_J[tmp][0];
                        if(l_p != 0 && l_q != 0)
                            array_radial_J[tmp][e1J][e2J][index_tmp_p][index_tmp_q] += lk2*lk4 * radial_2e_list_J[tmp][3]
                                    - 2.0*a4*lk2 * radial_2e_list_J[tmp][1] - 2.0*a2*lk4 * radial_2e_list_J[tmp][2];
                        else if(l_p != 0 && l_q == 0)
                            array_radial_J[tmp][e1J][e2J][index_tmp_p][index_tmp_q] -= 2.0*a4*lk2 * radial_2e_list_J[tmp][1];
                        else if(l_p == 0 && l_q != 0)
                            array_radial_J[tmp][e1J][e2J][index_tmp_p][index_tmp_q] -= 2.0*a2*lk4 * radial_2e_list_J[tmp][2];
                        array_radial_J[tmp][e1J][e2J][index_tmp_p][index_tmp_q] /= norm_J * 4.0 * pow(speedOfLight,2);
                    }
                    else if(intType == "LSSL")
                    {
                        array_radial_J[tmp][e1J][e2J][index_tmp_p][index_tmp_q] = 4.0*a2*a3 * radial_2e_list_J[tmp][0];
                        if(l_p != 0 && l_q != 0)
                            array_radial_J[tmp][e1J][e2J][index_tmp_p][index_tmp_q] += lk2*lk3 * radial_2e_list_J[tmp][3]
                                    - 2.0*a3*lk2 * radial_2e_list_J[tmp][1] - 2.0*a2*lk3 * radial_2e_list_J[tmp][2];
                        else if(l_p != 0 && l_q == 0)
                            array_radial_J[tmp][e1J][e2J][index_tmp_p][index_tmp_q] -= 2.0*a3*lk2 * radial_2e_list_J[tmp][1];
                        else if(l_p == 0 && l_q != 0)
                            array_radial_J[tmp][e1J][e2J][index_tmp_p][index_tmp_q] -= 2.0*a2*lk3 * radial_2e_list_J[tmp][2];
                ...
                for(int tmp = LmaxK; tmp >= 0; tmp -= 2)
                {
                    if(intType == "LSLS")
                    {
                        array_radial_K[tmp][e1K][e2K][index_tmp_p][index_tmp_q] = 4.0*a2*a4 * radial_2e_list_K[tmp][0];
                        if(l_p != 0 && l_q != 0)
                            array_radial_K[tmp][e1K][e2K][index_tmp_p][index_tmp_q] += lk2*lk4 * radial_2e_list_K[tmp][3] 
                                    - 2.0*a4*lk2 * radial_2e_list_K[tmp][1] - 2.0*a2*lk4 * radial_2e_list_K[tmp][2];
                        else if(l_p == 0 && l_q != 0)
                            array_radial_K[tmp][e1K][e2K][index_tmp_p][index_tmp_q] -= 2.0*a4*lk2 * radial_2e_list_K[tmp][1];
                        else if(l_p != 0 && l_q == 0)
                            array_radial_K[tmp][e1K][e2K][index_tmp_p][index_tmp_q] -= 2.0*a2*lk4 * radial_2e_list_K[tmp][2];
                        array_radial_K[tmp][e1K][e2K][index_tmp_p][index_tmp_q] /= norm_K * 4.0 * pow(speedOfLight,2);
                    }
                    else if(intType == "LSSL")
                    {
                        array_radial_K[tmp][e1K][e2K][index_tmp_p][index_tmp_q] = 4.0*a2*a3 * radial_2e_list_K[tmp][0];
                        if(l_q != 0)
                            array_radial_K[tmp][e1K][e2K][index_tmp_p][index_tmp_q] += lk2*lk3 * radial_2e_list_K[tmp][3] 
