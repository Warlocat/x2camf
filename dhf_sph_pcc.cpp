...
vVectorXd DHF_SPH::x2c2ePCC(vVectorXd* coeff2c)
{
    ...
    {
        for(int ir = 0; ir < occMax_irrep; ir++)
        for(int jj = 0; jj < fock_pcc[ir].size(); jj++)
        {
            fock_pcc[ir][jj] = 0.0;
        }
    }
...
vVectorXd DHF_SPH_CA::x2c2ePCC(vVectorXd* coeff2c)
{
    return DHF_SPH::x2c2ePCC(coeff2c);
    ...
    vector<vVectorXd> densityShells2c(NOpenShells+2);
    for(int ii = 0; ii < NOpenShells+1; ii++)
    {
        densityShells2c[ii].resize(occMax_irrep);
    }
    densityShells2c[NOpenShells+1].resize(irrep_list.size());
    ...
    for(int ir = 0; ir < occMax_irrep; ir++)
    {
        ...
        for(int ii = 0; ii < NOpenShells+2; ii++)
            densityShells2c[ii][ir] = evaluateDensity_aoc(coeff_2c[ir],occNumberShells[ii][ir],irrep_list[ir].size,true);
    ...
    for(int ir = 0; ir < occMax_irrep; ir++)
    {
        ...
        evaluateFock_2e(JK_x2c2c[ir],true,densityShells2c,irrep_list[ir].size,ir);
        fock_x2c2e_2e[ir] = X2C::transform_4c_2c(fock_4c_2e[ir], XXX[ir], RRR[ir], irrep_list[ir].size);
    }
    for(int ir = 0; ir < occMax_irrep; ir++)
    {
