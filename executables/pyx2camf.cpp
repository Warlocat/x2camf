                     ...
                     const Eigen::MatrixXd &exp_a)
{
    ...
    vMatrixXd amfi = scfer->get_amfi_unc(intor, twoC, "partialFock", Gaunt, gauge);
...
// Perturbative treatment of SOC within X2CAMF scheme
Eigen::MatrixXd amfi_pt(const int input_string, const int atom_number,
                        const int nshell, const int nbas,
                        const Eigen::MatrixXi &shell,
                        const Eigen::MatrixXd &exp_a)
{
    ...
    scfer->convControl = 1e-10;
    scfer->runSCF(twoC, renormS);
    vMatrixXd amfi = scfer->get_amfi_unc_4c(intor, twoC, "partialFock", Gaunt, gauge);
    MatrixXd amfi_all = Rotate::unite_irrep_4c(amfi, intor.irrep_list);
...
PYBIND11_MODULE(libx2camf, m)
{
    ...
    m.def("amfi_pt", &amfi_pt, "Compute the AMFI matrix for perturbative treatment");
}
