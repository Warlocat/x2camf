...
/*
    Evaluate different one-electron integrals in 2-spinor basis
    A fast version that computes S,T,V,W in one time
*/
void GTO_SPINOR::get_h1e_fast(MatrixXd& overlap, MatrixXd& kinetic, MatrixXd& Vnuc, MatrixXd& WWW, const bool& uncontracted_)
{
    int int_tmp = 0;
    if(!uncontracted_)
    {
        overlap.resize(size_gtoc_spinor, size_gtoc_spinor);
        overlap = MatrixXd::Zero(size_gtoc_spinor,size_gtoc_spinor);
        WWW = Vnuc = kinetic = overlap;
    }
    else
    {
        overlap.resize(size_gtou_spinor, size_gtou_spinor);
        overlap = MatrixXd::Zero(size_gtou_spinor,size_gtou_spinor);
        WWW = Vnuc = kinetic = overlap;
    }
    ...
    for(int ishell = 0; ishell < size_shell; ishell++)
    {
        ...
        {
            double kappa = (twojj + 1.0) * (ll - twojj/2.0);
            MatrixXd h1e_single_shell_s(size_gtos, size_gtos), h1e_single_shell_t(size_gtos, size_gtos), h1e_single_shell_v(size_gtos, size_gtos), h1e_single_shell_w(size_gtos, size_gtos);
            for(int ii = 0; ii < size_gtos; ii++)
            for(int jj = 0; jj < size_gtos; jj++)
            {
                double a1 = shell_list(ishell).exp_a(ii), a2 = shell_list(ishell).exp_a(jj);
                h1e_single_shell_s(ii,jj) = auxiliary_1e(2 + 2*ll, a1 + a2) / shell_list(ishell).norm(ii) / shell_list(ishell).norm(jj);
                h1e_single_shell_v(ii,jj) = -atomNumber * auxiliary_1e(1 + 2*ll, a1 + a2) / shell_list(ishell).norm(ii) / shell_list(ishell).norm(jj);
                h1e_single_shell_w(ii,jj) = 4*a1*a2 * auxiliary_1e(2*ll + 3, a1 + a2);
                h1e_single_shell_t(ii,jj) = 4*a1*a2 * auxiliary_1e(2*ll + 4, a1 + a2);
                if(ll!=0)
                {
                    h1e_single_shell_w(ii,jj) += pow(ll + kappa + 1.0, 2) * auxiliary_1e(2*ll-1, a1 + a2) - 2.0*(ll + kappa + 1.0)*(a1 + a2)*auxiliary_1e(2*ll + 1, a1 + a2);
                    h1e_single_shell_t(ii,jj) += pow(ll + kappa + 1.0, 2) * auxiliary_1e(2*ll, a1 + a2) - 2.0*(ll + kappa + 1.0)*(a1 + a2)*auxiliary_1e(2*ll + 2, a1 + a2);
                }
                h1e_single_shell_w(ii,jj) /= shell_list(ishell).norm(ii) * shell_list(ishell).norm(jj) / (double)-atomNumber;
                h1e_single_shell_t(ii,jj) /= 2.0 * shell_list(ishell).norm(ii) * shell_list(ishell).norm(jj);
            }

            if(!uncontracted_)
            {
                int size_subshell = shell_list(ishell).coeff.cols();
                MatrixXd int_1e_shell_s(size_subshell,size_subshell);
                MatrixXd int_1e_shell_t(size_subshell,size_subshell);
                MatrixXd int_1e_shell_v(size_subshell,size_subshell);
                MatrixXd int_1e_shell_w(size_subshell,size_subshell);
                for(int ii = 0; ii < size_subshell; ii++)
                for(int jj = 0; jj < size_subshell; jj++)
                {
                    int_1e_shell_s(ii,jj) = int_1e_shell_t(ii,jj) = int_1e_shell_v(ii,jj) = int_1e_shell_w(ii,jj) = 0.0;
                    for(int mm = 0; mm < size_gtos; mm++)
                    for(int nn = 0; nn < size_gtos; nn++)
                    {
                        int_1e_shell_s(ii,jj) += shell_list(ishell).coeff(mm, ii) * shell_list(ishell).coeff(nn, jj) * h1e_single_shell_s(mm,nn);
                        int_1e_shell_t(ii,jj) += shell_list(ishell).coeff(mm, ii) * shell_list(ishell).coeff(nn, jj) * h1e_single_shell_t(mm,nn);
                        int_1e_shell_v(ii,jj) += shell_list(ishell).coeff(mm, ii) * shell_list(ishell).coeff(nn, jj) * h1e_single_shell_v(mm,nn);
                        int_1e_shell_w(ii,jj) += shell_list(ishell).coeff(mm, ii) * shell_list(ishell).coeff(nn, jj) * h1e_single_shell_w(mm,nn);
                    }
                }
                for(int ii = 0; ii < size_subshell; ii++)
                for(int jj = 0; jj < size_subshell; jj++)
                for(int kk = 0; kk < twojj+1; kk++)
                {
                    overlap(int_tmp + kk + ii * (twojj+1), int_tmp + kk + jj * (twojj+1)) = int_1e_shell_s(ii,jj);
                    kinetic(int_tmp + kk + ii * (twojj+1), int_tmp + kk + jj * (twojj+1)) = int_1e_shell_t(ii,jj);
                    Vnuc(int_tmp + kk + ii * (twojj+1), int_tmp + kk + jj * (twojj+1)) = int_1e_shell_v(ii,jj);
                    WWW(int_tmp + kk + ii * (twojj+1), int_tmp + kk + jj * (twojj+1)) = int_1e_shell_w(ii,jj);
                }
                int_tmp += size_subshell * (twojj+1);
            }
            else
            {
                ...
                for(int kk = 0; kk < twojj+1; kk++)
                {
                    overlap(int_tmp + kk + ii * (twojj+1), int_tmp + kk + jj * (twojj+1)) = h1e_single_shell_s(ii,jj);
                    kinetic(int_tmp + kk + ii * (twojj+1), int_tmp + kk + jj * (twojj+1)) = h1e_single_shell_t(ii,jj);
                    Vnuc(int_tmp + kk + ii * (twojj+1), int_tmp + kk + jj * (twojj+1)) = h1e_single_shell_v(ii,jj);
                    WWW(int_tmp + kk + ii * (twojj+1), int_tmp + kk + jj * (twojj+1)) = h1e_single_shell_w(ii,jj);
                }
                int_tmp += size_gtos * (twojj+1);
            }
        }
...
MatrixXd GTO_SPINOR::get_h2e(const string& integralTYPE, const bool& uncontracted_) const
{
    ...
    for(int lshell = 0; lshell < size_shell; lshell++)
    {
        ...
        for(int twojj_l = abs(2*l_l-1); twojj_l <= 2*l_l+1; twojj_l = twojj_l + 2)
        {
            ...
            if(!uncontracted_)
            {
                cout << "contracted basis set spinor integral has not been implemented" << endl;
                exit(99);
            }
            
            double array_angular[Lmax+1][twojj_i + 1][twojj_j + 1][twojj_k + 1][twojj_l + 1], array_radial[Lmax+1][size_gtos_i][size_gtos_j][size_gtos_k][size_gtos_l];
            ...
            for(int tmp = Lmax; tmp >= 0; tmp = tmp - 2)
            {
                ...
                {                
                    array_angular[tmp][mi][mj][mk][ml] = int2e_get_angular(l_i, 2*mi-twojj_i, sym_ai, l_j, 2*mj-twojj_j, sym_aj, l_k, 2*mk-twojj_k, sym_ak, l_l, 2*ml-twojj_l, sym_al, tmp);
                }
                ...
                for(int ll = 0; ll < size_gtos_l; ll++)
                {
                    ...
                    if(integralTYPE == "LLLL")
                        array_radial[tmp][ii][jj][kk][ll] = int2e_get_radial_LLLL(l_i, k_i, shell_list(ishell).exp_a(ii), l_j, k_j, shell_list(jshell).exp_a(jj), l_k, k_k, shell_list(kshell).exp_a(kk), l_l, k_l, shell_list(lshell).exp_a(ll), tmp) / norm;
                    else if(integralTYPE == "SSLL")
                        array_radial[tmp][ii][jj][kk][ll] = int2e_get_radial_SSLL(l_i, k_i, shell_list(ishell).exp_a(ii), l_j, k_j, shell_list(jshell).exp_a(jj), l_k, k_k, shell_list(kshell).exp_a(kk), l_l, k_l, shell_list(lshell).exp_a(ll), tmp) / norm;
                    else if(integralTYPE == "LLSS")
                        array_radial[tmp][ii][jj][kk][ll] = int2e_get_radial_SSLL(l_k, k_k, shell_list(kshell).exp_a(kk), l_l, k_l, shell_list(lshell).exp_a(ll), l_i, k_i, shell_list(ishell).exp_a(ii), l_j, k_j, shell_list(jshell).exp_a(jj), tmp) / norm;
                    else if(integralTYPE == "SSSS")
                        array_radial[tmp][ii][jj][kk][ll] = int2e_get_radial_SSSS(l_i, k_i, shell_list(ishell).exp_a(ii), l_j, k_j, shell_list(jshell).exp_a(jj), l_k, k_k, shell_list(kshell).exp_a(kk), l_l, k_l, shell_list(lshell).exp_a(ll), tmp) / norm;
                    else if(integralTYPE == "SSLL_SF")
                        array_radial[tmp][ii][jj][kk][ll] = int2e_get_radial_SSLL_SF(l_i, k_i, shell_list(ishell).exp_a(ii), l_j, k_j, shell_list(jshell).exp_a(jj), l_k, k_k, shell_list(kshell).exp_a(kk), l_l, k_l, shell_list(lshell).exp_a(ll), tmp) / norm;
                    else if(integralTYPE == "LLSS_SF")
                        array_radial[tmp][ii][jj][kk][ll] = int2e_get_radial_SSLL_SF(l_k, k_k, shell_list(kshell).exp_a(kk), l_l, k_l, shell_list(lshell).exp_a(ll), l_i, k_i, shell_list(ishell).exp_a(ii), l_j, k_j, shell_list(jshell).exp_a(jj), tmp) / norm;
                    else if(integralTYPE == "SSSS_SF")
                        array_radial[tmp][ii][jj][kk][ll] = int2e_get_radial_SSSS_SF(l_i, k_i, shell_list(ishell).exp_a(ii), l_j, k_j, shell_list(jshell).exp_a(jj), l_k, k_k, shell_list(kshell).exp_a(kk), l_l, k_l, shell_list(lshell).exp_a(ll), tmp) / norm;
                    else if(integralTYPE == "SSLL_SD")
                        array_radial[tmp][ii][jj][kk][ll] = int2e_get_radial_SSLL_SD(l_i, k_i, shell_list(ishell).exp_a(ii), l_j, k_j, shell_list(jshell).exp_a(jj), l_k, k_k, shell_list(kshell).exp_a(kk), l_l, k_l, shell_list(lshell).exp_a(ll), tmp) / norm;
                    else if(integralTYPE == "LLSS_SD")
                        array_radial[tmp][ii][jj][kk][ll] = int2e_get_radial_SSLL_SD(l_k, k_k, shell_list(kshell).exp_a(kk), l_l, k_l, shell_list(lshell).exp_a(ll), l_i, k_i, shell_list(ishell).exp_a(ii), l_j, k_j, shell_list(jshell).exp_a(jj), tmp) / norm;
                    else if(integralTYPE == "SSSS_SD")
                        array_radial[tmp][ii][jj][kk][ll] = int2e_get_radial_SSSS_SD(l_i, k_i, shell_list(ishell).exp_a(ii), l_j, k_j, shell_list(jshell).exp_a(jj), l_k, k_k, shell_list(kshell).exp_a(kk), l_l, k_l, shell_list(lshell).exp_a(ll), tmp) / norm;
                    else
                    {
            ...
            for(int ll = 0; ll < size_gtos_l; ll++)
            {
                ...
                for(int ml = 0; ml < twojj_l + 1; ml++)
                {
                    ...
                    int_2e(eij,ekl) = 0.0;
                    for(int tmp = Lmax; tmp >= 0; tmp = tmp - 2)
                        int_2e(eij,ekl) += array_radial[tmp][ii][jj][kk][ll] * array_angular[tmp][mi][mj][mk][ml];
                }
            }
...
/*
    Evaluate different two-electron integrals (Coulomb) in 2-spinor basis
    A fast version that computes LLLL,SSLL,SSSS in one time
*/
void GTO_SPINOR::get_h2e_fast(MatrixXd& LLLL, MatrixXd& SSLL, MatrixXd& SSSS, const bool& uncontracted_)
{
    int size_2e, size_2e_2;
    if(!uncontracted_)
    {
    ...
    size_2e_2 = size_2e * size_2e;
    LLLL.resize(size_2e_2,size_2e_2);
    LLLL = MatrixXd::Zero(size_2e_2,size_2e_2);
    SSLL = SSSS = LLLL;
    ...
    for(int lshell = 0; lshell < size_shell; lshell++)
    {
        ...
        for(int twojj_l = abs(2*l_l-1); twojj_l <= 2*l_l+1; twojj_l = twojj_l + 2)
        {
            ...
            double array_angular[Lmax+1][twojj_i + 1][twojj_j + 1][twojj_k + 1][twojj_l + 1], array_radial_LLLL[Lmax+1][size_gtos_i][size_gtos_j][size_gtos_k][size_gtos_l], array_radial_SSLL[Lmax+1][size_gtos_i][size_gtos_j][size_gtos_k][size_gtos_l], array_radial_SSSS[Lmax+1][size_gtos_i][size_gtos_j][size_gtos_k][size_gtos_l];
            ...
            for(int tmp = Lmax; tmp >= 0; tmp = tmp - 2)
            {
                ...
                {
                    double norm = shell_list(ishell).norm(ii) * shell_list(jshell).norm(jj) * shell_list(kshell).norm(kk) * shell_list(lshell).norm(ll);
                    array_radial_LLLL[tmp][ii][jj][kk][ll] = int2e_get_radial_LLLL(l_i, k_i, shell_list(ishell).exp_a(ii), l_j, k_j, shell_list(jshell).exp_a(jj), l_k, k_k, shell_list(kshell).exp_a(kk), l_l, k_l, shell_list(lshell).exp_a(ll), tmp) / norm;
                    array_radial_SSLL[tmp][ii][jj][kk][ll] = int2e_get_radial_SSLL(l_i, k_i, shell_list(ishell).exp_a(ii), l_j, k_j, shell_list(jshell).exp_a(jj), l_k, k_k, shell_list(kshell).exp_a(kk), l_l, k_l, shell_list(lshell).exp_a(ll), tmp) / norm;
                    array_radial_SSSS[tmp][ii][jj][kk][ll] = int2e_get_radial_SSSS(l_i, k_i, shell_list(ishell).exp_a(ii), l_j, k_j, shell_list(jshell).exp_a(jj), l_k, k_k, shell_list(kshell).exp_a(kk), l_l, k_l, shell_list(lshell).exp_a(ll), tmp) / norm;
                }
            }
            for(int ii = 0; ii < size_gtos_i; ii++)
            for(int jj = 0; jj < size_gtos_j; jj++)
            ...
            for(int ll = 0; ll < size_gtos_l; ll++)
            {
                ...
                for(int ml = 0; ml < twojj_l + 1; ml++)
                {
                    ...
                    for(int tmp = Lmax; tmp >= 0; tmp = tmp - 2)
                    {   
                        LLLL(eij,ekl) += array_radial_LLLL[tmp][ii][jj][kk][ll] * array_angular[tmp][mi][mj][mk][ml];
                        SSLL(eij,ekl) += array_radial_SSLL[tmp][ii][jj][kk][ll] * array_angular[tmp][mi][mj][mk][ml];
                        SSSS(eij,ekl) += array_radial_SSSS[tmp][ii][jj][kk][ll] * array_angular[tmp][mi][mj][mk][ml];
                    }
                }
            }
            int_tmp2_l += loop_l * (twojj_l+1);
